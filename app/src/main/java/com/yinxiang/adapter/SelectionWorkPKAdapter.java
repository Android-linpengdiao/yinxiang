...
public class SelectionWorkPKAdapter extends BaseRecyclerAdapter<WorkData.DataBeanX.DataBean, ItemWorksLayoutBinding> {

    private static final String TAG = "WorkPKAdapter";
    ...
    public SelectionWorkPKAdapter(Context context) {
        super(context);
    }
    ...
    protected void onBindItem(ItemWorksLayoutBinding binding, final WorkData.DataBeanX.DataBean dataBean, final int position) {
        if (mList != null && mList.size() > 0) {
            ...
            if (activeId == dataBean.getActive_id()) {
                ...
                binding.selectionView.setBackground(mContext.getResources().getDrawable(R.drawable.button_white_t));
                binding.selectionView.setEnabled(false);
            }
            binding.selectionView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (onClickListener != null && activeId == dataBean.getActive_id()) {
                        Log.i(TAG, "onClick: ");
