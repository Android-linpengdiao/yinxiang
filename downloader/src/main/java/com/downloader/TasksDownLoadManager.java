...
public class TasksDownLoadManager {
    ...
    public List<TasksManagerModel> getAllTasks() {
        final Cursor c = TasksDownLoaderDBHelper.getInstance().getWritableDatabase().rawQuery("SELECT * FROM " + TasksManagerModel.TABLE_NAME, null);
    ...
    public TasksManagerModel addTaskToDB(final String url, final String path, final String name) {
        ...
        model.setUrl(url);
        model.setPath(path);
        Cursor cur = TasksDownLoaderDBHelper.getInstance().getWritableDatabase().rawQuery("SELECT * FROM " + TasksManagerModel.TABLE_NAME + " WHERE id='" + id + "'", null);
        if (!cur.moveToNext()) {
            final boolean succeed = TasksDownLoaderDBHelper.getInstance().getWritableDatabase().insert(TasksManagerModel.TABLE_NAME, null, model.toContentValues()) != -1;
    ...
    public void removeTaskToDB(String id) {
        TasksDownLoaderDBHelper.getInstance().getWritableDatabase().delete(TasksManagerModel.TABLE_NAME, TasksManagerModel.ID + " = ?", new String[]{id});
    }
